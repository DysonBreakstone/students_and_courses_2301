...
RSpec.describe Gradebook do
  before(:each) do
    ...
    @student_1 = Student.new({name: "A", grade: 11})
    @student_1.log_score(100)
    @student_1.log_score(90)
    @student_2 = Student.new({name: "B", grade: 22})
    @student_2.log_score(90)
    @student_2.log_score(80)
    @student_3 = Student.new({name: "C", grade: 33})
    @student_3.log_score(80)
    @student_3.log_score(70)
    @student_4 = Student.new({name: "D", grade: 44})
    @student_4.log_score(70)
    @student_4.log_score(60)
    @student_5 = Student.new({name: "E", grade: 55})
  ...
  it "can track all grades across all courses" do
    ...
    expect(@gradebook.all_grades[@course_1][students][@student_1]).to eq(95.0)
    expect(@gradebook.all_grades[@course_2][students][@student_4]).to eq(65.0)
  end
